var API = '', coinPage = 1;
var L = '<div class="loader w-5 h-5 border-2 border-gray-300 border-t-green-400 rounded-full mx-auto"></div>';
...
function fC(v) { if (v == null) return '--'; var c = v >= 0 ? 'text-green-500' : 'text-red-500'; return '<span class="' + c + '">' + (v >= 0 ? '+' : '') + v.toFixed(2) + '%</span>'; }
function spark(p, sym) {
  if (!p || p.length < 2) return '';
  var mn = Math.min.apply(null, p), mx = Math.max.apply(null, p), r = mx - mn || 1;
  // For stablecoins, make range wider to flatten line
  ...
  var cl = last >= first ? '#0ecb81' : '#f6465d';
  ...
  async function loadOverview() {
    ...
    try {
      ...
      if (btc.bitcoin.usd_24h_change) {
        ...
        document.getElementById('btcChange').className = 'text-xs mt-1 ' + (ch >= 0 ? 'text-green-500' : 'text-red-500');
      }
    } catch (e) { }
    try {
  ...
  async function loadCoins(page) {
    ...
    try {
      ...
      tb.innerHTML += coins.map(function (c, i) {
        ...
        return '<tr class="border-b border-gray-100 hover:bg-gray-50 transition cursor-pointer" onclick="location.href=\'/coin?symbol=' + sym + 'USDT\'"><td class="py-3 px-3 text-gray-400 text-xs">' + ((page - 1) * 50 + i + 1) + '</td><td class="py-3 px-3"><div class="flex items-center gap-2"><img class="coin-icon" src="' + (c.image || '') + '" onerror="this.style.display=\'none\'"><div><div class="font-medium text-gray-900">' + c.name + '</div><div class="text-gray-500 text-xs">' + sym + '</div></div></div></td><td class="py-3 px-3 text-right font-medium text-gray-900">' + fP(c.current_price) + '</td><td class="py-3 px-3 text-right">' + fC(c.price_change_percentage_1h_in_currency) + '</td><td class="py-3 px-3 text-right">' + fC(c.price_change_percentage_24h_in_currency) + '</td><td class="py-3 px-3 text-right">' + fC(c.price_change_percentage_7d_in_currency) + '</td><td class="py-3 px-3 text-right text-gray-500 hidden md:table-cell">' + fL(c.total_volume) + '</td><td class="py-3 px-3 text-right text-gray-500 hidden md:table-cell">' + fL(c.market_cap) + '</td><td class="py-3 px-3 hidden lg:table-cell">' + spark(c.sparkline_in_7d ? c.sparkline_in_7d.price : null, sym) + '</td><td class="py-3 px-3"><button class="bg-[#0ecb81] hover:bg-[#0ab06e] text-white text-xs font-semibold px-3 py-1.5 rounded-lg" onclick="event.stopPropagation();location.href=\'/coin?symbol=' + sym + 'USDT\'">TRADE</button></td></tr>';
      }).join('');
    } catch (e) {
  ...
  async function loadSidebarNews() {
    ...
    try {
      ...
      s.innerHTML = items.slice(0, 12).map(function (a) {
        ...
        var html = '<div class="pb-3 mb-3 border-b border-gray-100 last:border-b-0">';
        html += '<div class="text-gray-900 font-medium text-xs leading-tight hover:text-[#0ecb81] cursor-pointer">' + title + '</div>';
